...
%CURL% -s https://api.github.com/repos/SAM-BIM/SAM_Deploy/releases/latest --insecure | %~dp0grep\grep -o -P "(?<="""tag_name""":\s""").*(?=""")" > "%SAM_TEMP_DIR%\samversion"
...
if %noCurl%==0 (
 %CURL% --location --output "%SAM_TEMP_DIR%\SAM.zip" --url https://github.com/SAM-BIM/SAM_Deploy/releases/latest/download/SAM.zip --insecure
) else (
   %~dp0wget\wget.exe https://github.com/SAM-BIM/SAM_Deploy/releases/latest/download/SAM.zip -O "%SAM_TEMP_DIR%\SAM.zip"
)
